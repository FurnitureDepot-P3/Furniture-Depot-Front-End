...
export default function Bed() {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const reviewResponse = await axios.get(
        `http://localhost:3001/api/categories/products/reviews`
      );
  ...
  return bedroom && review ? (
    <div className="container">
      <Link to="/" className="back-btn" id="home-btn">
        {" "}
        ◁ Home{" "}
      </Link>
        ...
        {bedroom.map((products) => (
          <div className="product-card" key={products.name}>
            ...
            <p className="review-text">
              {" "}
              People who purchased this product rate it a{" "}
              {review.reviews[0].rating} out of 5!
            </p>
